...
class PreprocessinatorIncludesHandler
  ...
  constructor :configurator, :flaginator, :tool_executor, :task_invoker, :file_path_utils, :yaml_wrapper, :file_wrapper, :file_finder, :streaminator
  @@makefile_cache = {}
  ...
  def form_shallow_dependencies_rule(filepath:, subdir:, flags:, include_paths:, defines:)
    ...
    temp_filepath = @file_path_utils.form_temp_path(filepath, subdir, '_')
    ...
    @streaminator.stdout_puts("Extracting #include statements from #{File.basename(filepath)} via preprocessor...", Verbosity::NORMAL)
  ...
  def extract_includes(filepath:, subdir:, flags:, include_paths:, defines:)
    ignore_list = []
    ignore_list << filepath

    new_deps, _, _ =
      extract_includes_helper(
        filepath:      filepath,
        subdir:        subdir,
    ...
    return new_deps.uniq
  end

  def extract_includes_helper(filepath:, subdir:, include_paths:, ignore_list:, mocks:, flags:, defines:)
    # Extract the dependencies from the make rule
    make_rule = self.form_shallow_dependencies_rule(
      filepath:      filepath,
      subdir:        subdir,
      include_paths: include_paths,
      flags:         flags,
