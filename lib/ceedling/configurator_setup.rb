...
class ConfiguratorSetup
  ...
  def validate_threads(config)
    ...
    when Integer
      if compile_threads < 1
        @streaminator.stream_puts( ":project ↳ :compile_threads must be greater than 0", Verbosity::ERRORS )
        valid = false
      end
    when Symbol
      if compile_threads != :auto
        @streaminator.stream_puts( ":project ↳ :compile_threads is neither an integer nor :auto", Verbosity::ERRORS ) 
        valid = false
      end
    else
      @streaminator.stream_puts( ":project ↳ :compile_threads is neither an integer nor :auto", Verbosity::ERRORS ) 
    ...
    when Integer
      if test_threads < 1
        @streaminator.stream_puts( ":project ↳ :test_threads must be greater than 0", Verbosity::ERRORS )
        valid = false
      end
    when Symbol
      if test_threads != :auto
        @streaminator.stream_puts( ":project ↳ :test_threads is neither an integer nor :auto", Verbosity::ERRORS ) 
        valid = false
      end
    else
      @streaminator.stream_puts( ":project ↳ :test_threads is neither an integer nor :auto", Verbosity::ERRORS ) 
  ...
  def validate_plugins(config)
    ...
    missing_plugins.each do |plugin|
      @streaminator.stream_puts("Plugin '#{plugin}' not found in built-in or project Ruby load paths. Check load paths and plugin naming and path conventions.", Verbosity::ERRORS)
    end
