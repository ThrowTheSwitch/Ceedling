...
class ReleaseBuildExecutor
  ...
  def compile_objects(state)
    ...
    @batchinator.exec(workload: :compile, things: state.objects) do |object|
      ...
      # Worker threads share `skipped` -- a bare `+= 1` here is a read-modify-write
      # across threads and can silently lose an increment under MRI's GVL, same as
      # the test pipeline's own equivalent counter (test_build_executor.rb).
  ...
  def artifactinate(state)
    ...
    artifacts.each do |file|
      # `file` was just linked/generated by this same build -- exist_with_retry? rides
      # out it briefly not yet being visible right after that shell-out exits.
      @file_wrapper.cp( file, @configurator.project_release_artifacts_path ) if @file_wrapper.exist_with_retry?( file )
    end
  end
  ...
  def compile_release_component(object:, state:)
    ...
    # recording this target's new baseline. exist_with_retry? rides out the file
    # briefly not yet being visible right after the shell-out exits.
