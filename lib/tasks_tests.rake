...
namespace TESTS_TASKS_ROOT_NAME.to_sym do
  ...
  task :all => [:directories] do
    @ceedling[:test_invoker].invoke_tests(COLLECTION_ALL_TESTS)
  end
  ...
  COLLECTION_ALL_TESTS.each do |test|
    ...
    task name.to_sym => [:directories] do
      @ceedling[:test_invoker].invoke_tests(test)
    end
  ...
  task :delta => [:directories] do
    @ceedling[:test_invoker].invoke_tests(COLLECTION_ALL_TESTS, {:force_run => false})
  end
