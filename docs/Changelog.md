...
NOTE: The ReportGenerator tool does not support extracting MC/DC even if those metrics are available in the coverage data dumps it processes. The GCov plugin can only produce MC/DC reporting through its simplistic console reports and reports generated by GCovr.
