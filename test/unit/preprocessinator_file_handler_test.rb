...
class PreprocessinatorFileHandlerTest < Test::Unit::TestCase
  ...
  def setup
    objects = create_mocks(:preprocessinator_file_handler_helper, :configurator, :tool_executor, :file_path_utils, :file_wrapper)
  ...
  should "preprocess a file" do
    ...
    @preprocessinator_file_handler_helper.expects.extract_base_file_from_preprocessed_expansion('project/build/preprocessed/files/file.c').returns(['', 'void foo(void)', '{', '}'])
