...
describe CExtractorPreprocessing do
  ...
  context "#try_extract_typedef" do
    ...
    def try_typedef(text)
      scanner = StringScanner.new(text)
      result  = @preprocessing.try_extract_typedef(scanner)
      [result, scanner.pos]
    end
    ...
    it "does not advance scanner on failure" do
      scanner = StringScanner.new('int x;')
      @preprocessing.try_extract_typedef(scanner)
      expect(scanner.pos).to eq 0
    end
    ...
    # --- Comments and string literals must not break extraction ---

    it "preserves a line comment inside the typedef body" do
      input = "typedef struct {\n  int x; // x coordinate\n  int y; // y coordinate\n} Point;\n"
      result, pos = try_typedef(input)
      expect(result).to eq [true, input]
      expect(pos).to eq input.length
    ...
    it "preserves a block comment inside the typedef body" do
    ...
    it "handles a block comment containing a semicolon — does not terminate early" do
